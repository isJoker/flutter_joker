...
class HomeState extends State<HomePage> {
  ...
  void _httpClient() async {
    ...
    if (response.statusCode == 200) {
      responseBody = await response.transform(convert.utf8.decoder).join();

      var convertDataToJson = convert.jsonDecode(responseBody)["data"]["datas"];
      setState(() {
  ...
  void _pullNet() async {
    await http
      ...
      if (response.statusCode == 200) {
        var convertDataToJson = convert.jsonDecode(response.body);
